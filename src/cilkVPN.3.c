/*
clang ./cilkVPN.3.c -O3 -o ./cilkVPN.3

Peer 1 keypair:
ff4cb24de001f7970abe632ac9eeda89452b71042b71ef8959978a49e534a7e4
eb119f5604eac33290f6d8a0dc0e52d82f17914995c4acc7a8211b2235a71fb1

Peer 2 keypair:
9c5904a620c3ecfb88ff19c03f3d05ef709f66f7f0929fd9d3ddf373333012cf
e911c20b180ea9316e93f1e94e88269bdb0290cf24bed6f0fefb939350cfefad

Peer 3 keypair:
1ce257003246bc455931281ed68adcc1dff03a12bc1e864d810f6e3d491a8966
e8c93c2a3426ea3a0e5b03dd7bb495725ddf13e23abdbf2997dba18f521d5dd0
...
#define CILK_VPN_HANDSHAKE_DATAGRAM (CILK_VPN_TYPE_DATAGRAM + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES + crypto_onetimeauth_BYTES + crypto_sign_BYTES + CILK_VPN_HANDSHAKE_TO_SIGN)
...
#define CILK_VPN_INBOUND_UDP_BUFFER 65535
#define CILK_VPN_MAX_ALLOWED_IPS 32
...
typedef enum {
    HANDSHAKE_DATAGRAM = 0x00,
    DATAGRAM           = 0x01,
    KEEPALIVE_DATAGRAM = 0x02
...
typedef struct device_t {
    ...
    unsigned char outbound_handshake_datagram[CILK_VPN_HANDSHAKE_DATAGRAM];
    
    int inbound_recv_bytes;
    unsigned char inbound_encrypted_datagram[CILK_VPN_INBOUND_UDP_BUFFER];
    unsigned char inbound_decrypted_datagram[CILK_VPN_INBOUND_UDP_BUFFER];
    unsigned char inbound_symmetric_key[crypto_box_BEFORENMBYTES];
...
device* make_device_from_config(const char* config_file) {
    ...
    memcpy(d->outbound_handshake_to_sign + crypto_sign_PUBLICKEYBYTES, &d->address.ip_addr, CILK_VPN_HANDSHAKE_TO_SIGN_IP);
...
int send_handshake_datagram_2_peer(device* d, peer* p) {
    ...
    memset(d->outbound_handshake_datagram, 0, CILK_VPN_HANDSHAKE_DATAGRAM);
    d->outbound_handshake_datagram[0] = HANDSHAKE_DATAGRAM;
    
    unsigned char* outbound_handshake_nonce = d->outbound_handshake_datagram + CILK_VPN_TYPE_DATAGRAM;
    randombytes(outbound_handshake_nonce, crypto_box_NONCEBYTES); //gen nonce
    
    unsigned char* box = d->outbound_handshake_datagram + CILK_VPN_TYPE_DATAGRAM + crypto_box_NONCEBYTES + crypto_box_BOXZEROBYTES; //skip 1 byte of transport header + 24 bytes of nonce + 16 zerobytes
    ...
    memcpy(d->outbound_handshake_datagram + CILK_VPN_TYPE_DATAGRAM + crypto_box_NONCEBYTES, d->x25519_ephemeral_keypair.public_key, crypto_box_PUBLICKEYBYTES); //replace 32 zero bytes on ephemeral public key
    
    return sendto(d->udp, d->outbound_handshake_datagram, CILK_VPN_HANDSHAKE_DATAGRAM, 0, (struct sockaddr *)&p->endpoint_addr, sizeof(p->endpoint_addr));
}

void handle_handshake_datagram(device* d) {
    ...
    memset(d->inbound_decrypted_datagram, 0, CILK_VPN_INBOUND_UDP_BUFFER);
    crypto_box_open_afternm(d->inbound_decrypted_datagram, box, inbound_unboxed_datagram_len, nonce, d->inbound_symmetric_key); //decrypt
    ...
    if (p->init_handshake == 0) {
        if (send_handshake_datagram_2_peer(d, p) <= 0) {
            //log
        }
    } else {
        p->init_handshake = 0;
...
void print_inbound_message(device* d){
    char client_ip[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &d->inbound_peer_addr.sin_addr, client_ip, INET_ADDRSTRLEN);
    
    printf("Received %d bytes from %s:%d\n", d->inbound_recv_bytes, client_ip, ntohs(d->inbound_peer_addr.sin_port));
    
    for (int i = 0; i< d->inbound_recv_bytes; i++) {
        printf("%02x ", d->inbound_encrypted_datagram[i]);
...
void cilkVPN__recv(device* d) {
    memset(d->inbound_encrypted_datagram, 0, CILK_VPN_INBOUND_UDP_BUFFER);
    d->inbound_recv_bytes = 0;
    
    d->inbound_recv_bytes = recvfrom(d->udp, d->inbound_encrypted_datagram, CILK_VPN_INBOUND_UDP_BUFFER, 0, (struct sockaddr *)&d->inbound_peer_addr, &d->inbound_peer_addr_len);
    if (d->inbound_recv_bytes < 0) {
        return;
    }
    
    print_inbound_message(d);
    
    if (d->inbound_encrypted_datagram[0] == HANDSHAKE_DATAGRAM) {
        handle_handshake_datagram(d);
        print_peers(d);
    } else if (d->inbound_encrypted_datagram[0] == DATAGRAM) {
        //datagram CILK_VPN_DATAGRAM = CILK_VPN_TYPE_DATAGRAM + crypto_box_NONCEBYTES + CILK_VPN_PEER_INDEX + crypto_onetimeauth_BYTES + ip package
    } else if (d->inbound_encrypted_datagram[0] == KEEPALIVE_DATAGRAM) {
        //keepalive
    }
}
...
int main(int argc, char **argv) {
    ...
    while (p) {
        if (p->has_endpoint) {
            p->init_handshake = 1;
            send_handshake_datagram_2_peer(d, p);
        }
